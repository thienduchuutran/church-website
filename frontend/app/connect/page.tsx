...
export default async function ConnectPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {/* Location */}
      <section className="mb-10 rounded-xl border border-border bg-surface/50 p-8">
        <h2 className="mb-3 text-2xl font-semibold text-foreground">{s.location_heading}</h2>
        <p className="text-foreground">{s.location_address}</p>
        <p className="text-foreground">{s.location_city_state_zip}</p>
        <p className="mt-3 text-sm text-muted">{s.location_directions_note}</p>
      </section>

      {/* Contact */}
      <section className="mb-10 rounded-xl border border-border bg-surface/50 p-8">
        <h2 className="mb-4 text-2xl font-semibold text-foreground">{s.contact_heading}</h2>
        <dl className="space-y-2">
          <div className="flex gap-3">
            <dt className="w-20 text-sm font-medium text-muted">Email</dt>
            <dd className="text-foreground">{s.contact_email}</dd>
          </div>
          <div className="flex gap-3">
            <dt className="w-20 text-sm font-medium text-muted">Phone</dt>
            <dd className="text-foreground">{s.contact_phone}</dd>
          </div>
        </dl>
        <p className="mt-4 text-sm text-muted">{s.contact_note}</p>
      </section>
      ...
      <section className="rounded-xl border border-border bg-surface/50 p-8">
        ...
        <p className="leading-relaxed text-muted">{s.plan_a_visit_body}</p>
      </section>
    </div>
  )
}
