    ...
    <header className="sticky top-0 z-50 border-b border-border bg-surface/80 backdrop-blur-lg">
      <nav aria-label="Primary" className="mx-auto max-w-6xl">
            ...
            return (
              ...
              >
                <button
                  ...
                  className={`inline-flex min-h-11 items-center gap-1 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                    isActive
                      ? 'bg-primary/10 text-primary'
                      : 'text-muted hover:bg-primary/5 hover:text-primary'
                  }`}
                ...
                <ul
                  id={panelId}
                  role="list"
                  className={`absolute left-0 top-full z-50 mt-0 min-w-[10rem] rounded-lg border border-border bg-surface p-1 shadow-md ${
                    isOpen ? 'block' : 'hidden'
                  }`}
                >
                  {item.children.map(({ href, label }) => (
                    <li key={href}>
                      <Link
                        href={href}
                        className={`block min-h-11 rounded-md px-3 py-2 text-sm font-medium leading-snug transition-colors ${
                          pathname === href
                            ? 'bg-primary/10 text-primary'
                            : 'text-muted hover:bg-primary/5 hover:text-primary'
                        }`}
        ...
        <div className="flex items-center gap-2">
          <Link
            href={connectHref}
            className={`hidden min-h-11 items-center justify-center rounded-lg px-3 text-sm font-semibold transition-colors md:inline-flex ${
      ...
      >
            ...
            return (
              <li key={item.href}>
                <Link
                  ...
                  className={`block min-h-11 rounded-lg px-3 py-2 text-sm font-medium leading-snug transition-colors ${
                    pathname === item.href
                      ? 'bg-primary/10 text-primary'
                      : 'text-muted hover:bg-primary/5 hover:text-primary'
                  }`}
        ...
        <li>
          <Link
            href={connectHref}
            onClick={() => setMobileOpen(false)}
            className={`block min-h-11 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
              pathname === connectHref
                ? 'bg-primary/10 text-primary'
                : 'text-muted hover:bg-primary/5 hover:text-primary'
            }`}
