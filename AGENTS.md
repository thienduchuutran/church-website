...
| Calendar Locations strip, venue names, `calendar_places`, address dedupe, why one address prints twice, AI place naming | `docs/agents/backend.md` → "Place naming" + `docs/agents/frontend.md` → "The Locations strip is a list of places" + `docs/agents/known-quirks.md` ("The Locations strip shows two rows for what is obviously one address") |
| Editing address matching / `NormalizeAddressKey` / adding a street or state abbreviation | `backend/internal/model/address.go` + its test. **Never reimplement this in TypeScript** - the DB's `address_key` UNIQUE constraint depends on there being exactly one normalizer |
